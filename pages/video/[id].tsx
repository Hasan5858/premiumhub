...
import { fetchVideoData, fetchCreators } from "@/services/api"
...
export default function VideoPage() {
  ...
  const { id, from, creatorSlug, page, webseriesSlug, webseriesPage, categorySlug, categoryPage, videoData: videoDataParam } = router.query
  ...
  const getBackUrl = () => {
    if (from === "creator" && creatorSlug) {
      // Back to creator page
      const pageParam = page ? `?page=${page}` : ""
      return `/creator/${creatorSlug}${pageParam}`
    } else if (from === "webseries" && webseriesSlug) {
